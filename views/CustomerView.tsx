...
import { ShoppingCart, Search, Menu, Sun, Moon, LogOut, ClipboardList, Eye, CheckCircle, Download, Send, Filter, Star, Flame, Clock } from 'lucide-react';
...
export const CustomerView: React.FC = () => {
  ...
  const generateReceipt = () => {
    ...
    doc.text("FoodieQ", 105, y, { align: "center" });
    y += 10;
    doc.setFontSize(10);
    doc.setTextColor(0);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B1120] pb-24 md:pb-0">
      ...
      <header className="sticky top-0 z-40 glass border-b-0">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={toggleTheme} className="p-2.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-yellow-400 transition-colors">
              {theme === 'dark' ? <Sun size={20} /> : <Moon size={20} />}
            </button>
            <button onClick={() => setIsSidebarOpen(true)} className="p-2.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 lg:hidden">
      ...
      <main className="max-w-7xl mx-auto px-4 pt-6">
        ...
        <div className="relative w-full rounded-3xl overflow-hidden mb-8 shadow-2xl shadow-orange-500/20 group">
          <div className="absolute inset-0 bg-gradient-to-r from-[#ff6b35] via-[#ff8e53] to-[#ff4757]"></div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between p-6 md:p-10 text-white h-full min-h-[220px]">
            <div className="md:w-1/2 mb-6 md:mb-0 z-10">
              <div className="inline-block px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-xs font-bold mb-3 border border-white/30">
                🔥 Hot & Fresh
              </div>
            ...
            <div className="absolute right-[-20px] bottom-[-30px] md:right-10 md:bottom-[-40px] transform md:rotate-[-10deg] transition-transform duration-700 group-hover:scale-105 group-hover:rotate-0">
        ...
        <div className="relative mb-8 group z-20">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-4 rounded-2xl bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 shadow-lg shadow-slate-200/50 dark:shadow-none focus:ring-0 focus:border-primary transition-all dark:text-white"
            value={searchQuery}
        ...
        <div className="flex gap-3 overflow-x-auto pb-6 hide-scrollbar">
          {categories.map(cat => (
            <button
              ...
              className={`whitespace-nowrap px-6 py-3 rounded-2xl font-bold text-sm transition-all duration-300 transform ${
                selectedCategory === cat 
                  ? 'bg-slate-900 dark:bg-white text-white dark:text-slate-900 shadow-xl shadow-slate-900/20 scale-105 ring-2 ring-offset-2 ring-slate-900 dark:ring-white dark:ring-offset-slate-900' 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-6 gap-y-8">
          {filteredItems.map(item => (
            <div 
              key={item.id} 
              className="group bg-white dark:bg-slate-800 rounded-[2rem] p-4 hover:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] dark:hover:shadow-none transition-all duration-300 cursor-pointer border border-slate-100 dark:border-slate-800 hover:border-primary/30 dark:hover:border-primary/30 relative flex flex-col"
              onClick={() => setArItem(item)}
            >
              {/* Image Section - Pops out on hover */}
              <div className="h-48 w-full flex items-center justify-center mb-4 relative bg-slate-50 dark:bg-slate-900/50 rounded-[1.5rem] overflow-hidden">
                ...
                {/* Rating Badge */}
                <div className="absolute top-3 right-3 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm px-2.5 py-1 rounded-full text-xs font-bold shadow-sm flex items-center gap-1">
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-full max-w-sm px-4">
        ...
        >
          ...
          <div className="flex items-center gap-2 font-bold bg-white text-slate-900 px-4 py-2 rounded-xl text-sm">
             Lihat Troli <ChevronRight size={16} />
          </div>
        </button>
      </div>

      {/* Sidebars and Modals */}
      <aside className={`fixed top-0 left-0 h-full w-72 bg-white dark:bg-[#0B1120] z-50 transform transition-transform duration-300 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} shadow-2xl`}>
         <div className="p-8">
           <div className="w-20 h-20 bg-gradient-to-br from-orange-100 to-orange-200 dark:from-slate-800 dark:to-slate-700 rounded-full flex items-center justify-center text-4xl mb-4 shadow-inner">👤</div>
           <h3 className="font-bold text-2xl dark:text-white leading-tight">{user?.name}</h3>
           <p className="text-slate-500 text-sm mb-8 font-medium">{user?.phone}</p>
           
           <nav className="space-y-2">
             <button onClick={() => { setCurrentView('my-orders'); setIsSidebarOpen(false); }} className="w-full flex items-center gap-4 px-4 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-200 font-bold hover:bg-orange-50 dark:hover:bg-slate-800 transition-colors">
               <ClipboardList size={22} className="text-orange-500" /> Pesanan Saya
             </button>
      ...
      {successOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-lg animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 w-full max-w-sm text-center relative overflow-hidden animate-scale-in shadow-2xl">
            ...
            <div className="space-y-3">
              ...
              <Button onClick={sendWhatsApp} variant="secondary" className="w-full py-4 text-base rounded-2xl bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 border border-[#25D366]/20" icon={<Send size={20} />}>WhatsApp Admin</Button>
              <button onClick={() => setSuccessOrder(null)} className="w-full py-4 text-sm font-bold text-slate-400 hover:text-slate-600 transition-colors">Tutup</button>
            </div>
          </div>
        </div>
      )}
